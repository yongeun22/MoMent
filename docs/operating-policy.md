...
- 삭제는 `DELETE /api/traces`와 생성형 고엔트로피 삭제 토큰으로만 처리한다.
- `py -3.13 manage.py generate-operator-token --purpose trace-delete`로 토큰과 검증값을 생성한다.
- 토큰 원문은 비밀번호 관리자에만 저장하고 코드, 문서, 커밋, README, 이슈, 채팅 기록에 남기지 않는다.
- 운영 환경에는 `TRACE_DELETE_TOKEN_HASH`를 설정한다. 로컬 확인에는 `MOMENT_TRACE_DELETE_TOKEN_HASH`를 사용할 수 있다.
- 잘못된 id 또는 토큰은 `Not found.`로 응답하게 둔다.
- 삭제 API는 작성 API와 별도로 rate limit을 둔다. 여러 번 실패하면 올바른 비밀번호를 보내도 제한 시간이 끝날 때까지 삭제되지 않는다.

운영 절차:

1. `GET /api/traces` 응답에서 삭제할 항목의 `id`를 확인한다.
2. 비밀번호 관리자에서 삭제 토큰을 가져온다.
3. `DELETE /api/traces`에 `{ "id": 123, "token": "<high-entropy-token>" }` JSON을 보낸다.
...
- `py -3.13 manage.py generate-operator-token --purpose status-update`로 삭제용과 별도의 토큰을 생성하고 Cloudflare Pages 환경에 `STATUS_UPDATE_TOKEN_HASH`를 설정한다.
- 함수 코드에는 공개 fallback 해시가 없으므로 Secret이 설정되지 않으면 POST가 허용되지 않는다.
...
- Functions를 배포하기 전에 D1 백업을 만든다.
- `migrations/README.md` 절차로 적용되지 않은 마이그레이션을 먼저 적용한다.
- 공개 GET 요청에서는 스키마 생성이나 moderation 삭제를 실행하지 않는다.
- 마이그레이션과 Functions 코드는 같은 배포 단위로 검토한다.
